...
NSString *const kGPUImageFLChromaKeyBlendFilterFragmentShaderString = SHADER_STRING
(
 ...
 void main()
{
    ...
    if (blend < .5)
    {
        overlayer = texture2D(inputImageTexture, textureCoordinate);
        vec2 displaceCoord = mix(textureCoordinate2, overlayer.br, displacement);
        base = texture2D(inputImageTexture2, displaceCoord);
    }else{
        overlayer = texture2D(inputImageTexture2, textureCoordinate2);
        vec2 displaceCoord = mix(textureCoordinate, overlayer.br, displacement);
        base = texture2D(inputImageTexture, displaceCoord);
    }
    ...
    blendValue = min(blendValue, maxBlend);
    ...
    vec4 col = mix(base, overlayer, blendValue);
    gl_FragColor = col;
}
 );
#endif

@interface FLChromaKeyBlendFilter()
...
@implementation FLChromaKeyBlendFilter

- (id)init
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageFLChromaKeyBlendFilterFragmentShaderString]))
    {
        return nil;
    }
